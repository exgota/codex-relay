...
4. **Supervise.** Act on the returned `state` (table below). While the state is `working`, run `RELAY wait <task> --timeout 540` again. Each wait resumes where the last one stopped. It returns the newest three Codex messages it hasn't returned before, each trimmed to its last 2000 characters. Use `read` when you need more.
